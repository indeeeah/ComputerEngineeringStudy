...
void addNode(Node** target, int index, Edge* edge) {    // 특정한 인덱스의 정점에 특정한 간선이 있다는 함수
...
int main(void) {
    ...
    pq->count = 0;
    for (int i = 0 ; i < m ; i++) {
        int a, b, c;    // a에서 b만큼가는데 c(가중치)만큼 걸린다.
        // 무방향 그래프라고 가정
        scanf("%d %d %d", &a, &b, &c);
    ...
    for (int i = 1 ; i <= n ; i++) {
        ...
        // 방문하지 않은 노드 즉 처리하지 않은 노드 중에서 가장 비용이 적은 노드를 찾아준다.
        // 방문하지 않은 노드를 찾는다.
        while (1) {
            Edge* now = pop(pq);
            if (now == NULL) {
        ...
        // 그 노드에 인접한 모든 노드에 방문하면서 우선순위 큐에 넣어준다.
        while (cur != NULL) {
            push(pq, cur->data);
            cur = cur->next;
        }
